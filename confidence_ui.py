...
def main(subsample=1, n_stop=24000,
    ...
    for index, a_file in enumerate(file_names):
        ...
        heatmaps_human.append(model_human.make_heatmaps_once(model_human.outputs[index]))
        heatmaps_pose.append(model_pose.make_heatmaps_once(model_pose.outputs[index]))
        ...
        save_frame_result(outpath_human, image, heatmaps_human[index], conf_human,
                          model_human.joint_names, model_human.model_name)
        save_frame_result(outpath_pose, image, heatmaps_pose[index], conf_pose,
                          model_human.joint_names, model_pose.model_name)
        save_frame_result_dual(outpath_dual, image, [heatmaps_human[index], heatmaps_pose[index]],
                               [conf_human, conf_pose], model_human.joint_names,
                               [model_human.model_name, model_pose.model_name])

    # plot final image
    fig = plt.figure(figsize=(12, 8))
    outer_canvas = grid.GridSpec(1, 2)

    # plot a frame on the left: mean heatmaps
    n_parts = len(heatmaps_human[0])  # check how many sub heatmaps we have
    cum_heatmap_human = [np.zeros((3, 3)) for _ in range(n_parts)]  # empty matrices
    cum_heatmap_pose = [np.zeros((3, 3)) for _ in range(n_parts)]
    for human_heatmap, pose_heatmap in zip(heatmaps_human, heatmaps_pose):
        for index, (human_submap, pose_submap) in enumerate(zip(human_heatmap, pose_heatmap)):
            # resize cumulative heatmap to new addition if needed
            cum_heatmap_human[index], human_submap = extend_matrices(cum_heatmap_human[index], human_submap)
            cum_heatmap_pose[index], pose_submap = extend_matrices(cum_heatmap_pose[index], pose_submap)
            cum_heatmap_human[index] += human_submap
            cum_heatmap_pose[index] += pose_submap

    used_frame = model_human.input_list[-1]  # last frame, whatever you want to show
    image = imread(used_frame)
    # plot heatmaps in first grid
    left_canvas = grid.GridSpecFromSubplotSpec(4, 2, subplot_spec=outer_canvas[0])
    for num, a_heatmap in enumerate(cum_heatmap_human):
        ax = plt.Subplot(fig, left_canvas[num])
        ax.set_title(model_human.joint_names[num])
        ax.axis('off')
